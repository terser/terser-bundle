...
async function buildGraph (roots, { resolve }) {
  ...
  // Calculate need for all-exports object
  await traverse({
    resolve,
    graph,
    entry: ':root'
    ...
    const node = graph.node(module.src)
    ...
    const edge = graph.edge(parent.src, module.src)
...
// TODO make this not resolve the module to save memory. instead, yield graph nodes?
async function traverse ({ resolve, graph, entry }, iterate) {
  if (entry === ':root') {
    for (const { w: entrypoint } of graph.outEdges(entry)) {
      await traverse({ resolve, graph, entry: entrypoint }, iterate)
    }
    return
  }
  const visited = new Set()
